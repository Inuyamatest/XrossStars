...
(function (root, factory) {
  ...
  function createDefaultRuleConfig() {
    return {
      ...
      // 使われなかったアタック強化（メモリア・タクティクス等が「次のアタック」に積んだダメージ+と、次のアタックに紐づくアタック後効果）
      pendingAttackEffectsExpiryPolicy: {
        // 終了フェイズでプレイエリアのカードがトラッシュに置かれるとき、そのプレイヤーの分を消す（次のターンへ持ち越さない）
        expiresAtEndPhase: true,
        // ラウンド終了時（プレイエリア・手札がトラッシュへ）も、両プレイヤーの分を消す
